...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(shader->program, "Bones"), mesh->m_BoneInfo.size(), GL_FALSE, (const GLfloat *)bones);
...
void Lab7::ComputeInterpolatedRotation(aiQuaternion& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    aiQuaternion::Interpolate(out, pNodeAnim->mRotationKeys[rotationIndex].mValue, pNodeAnim->mRotationKeys[nextRotationIndex].mValue, factor);
    out.Normalize();
}
...
void Lab7::ComputeInterpolatedScaling(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    out += factor * (pNodeAnim->mScalingKeys[nextScalingIndex].mValue - pNodeAnim->mScalingKeys[scalingIndex].mValue);
}
...
void Lab7::ComputeInterpolatedPosition(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    out += factor * (pNodeAnim->mPositionKeys[nextPositionIndex].mValue - pNodeAnim->mPositionKeys[positionIndex].mValue);
}
